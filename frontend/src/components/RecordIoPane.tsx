...
export function RecordIoPane({ trace, position, recordIndex, onSource }: Props) {
  // the trace already carries this stage's input and output, and unlike the
  // records table it is not limited to the page of records that is loaded
  const event = trace?.events.find((candidate) => candidate.position === position) ?? null;

  return (
    <section style={{ borderTop: "1px solid var(--border)" }}>
      {/* the trace pane reports a trace that failed to load, so this pane only
      ...
      {recordIndex !== null && trace && !event && (
        <Empty>{`record ${recordIndex} never reached this stage`}</Empty>
      )}
      {event && (
        <div
          style={{
